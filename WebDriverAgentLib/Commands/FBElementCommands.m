...
+ (id<FBResponsePayload>)handleDragCoordinate:(FBRouteRequest *)request
{
  ...
  XCUICoordinate *endCoordinate = [self.class gestureCoordinateWithOffset:endOffset
                                                                  element:session.activeApplication];
//  取消pressForDuration，将duration改成velocity,值越大，滑动越快 (最小值：50)
  NSTimeInterval duration = [request.arguments[@"duration"] doubleValue];
  if (duration<50){
    duration = 50;
  }
  [startCoordinate pressForDuration:0 thenDragToCoordinate:endCoordinate withVelocity:duration thenHoldForDuration:0];
  return FBResponseWithOK();
}
